      ...
      <div className="container max-w-7xl mx-auto">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-8">
            ...
            <div className="mission-card text-left p-6 bg-gradient-to-br from-primary/10 to-primary/5 rounded-2xl border border-primary/50 backdrop-blur-lg">
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {stats.map((stat, index) => (
                <div
                  key={index}
                  className="stat-card text-center p-4 bg-card/50 rounded-2xl border border-primary/20 backdrop-blur-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_25px_hsla(var(--primary),0.2)]"
                >
                  <div className="flex items-center justify-center w-16 h-16 mx-auto mb-3 bg-primary/10 border border-primary/20 rounded-xl">
                    {stat.icon}
                  </div>
                  <p className="text-3xl font-bold text-primary">
                    {stat.isYear ? stat.value : <Counter target={stat.value} />}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1 h-10 flex items-center justify-center">
                    {stat.description || stat.label}
                  </p>
                </div>
              ))}
            </div>
            <div className="testimonial p-8 bg-card/50 rounded-2xl border border-primary/20 backdrop-blur-lg">
              <p className="text-lg italic mb-6 text-foreground/90 leading-relaxed">
